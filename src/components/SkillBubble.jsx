...
export default function SkillBubble({ logo, parentRef: ref }) {
    ...
    //         setX(Math.floor(Math.random() * 400))
    //         setY(Math.floor(Math.random() * 220))
    //     }, 5000);
    ...
    return (
        <AnimatePresence>
            {isInView && <motion.div
                ...
                className={`absolute shrink-0 ${logo.color} p-2 flex justify-center items-center flex-col gap-1 rounded-full bg-sky-400/20 border border-sky-600 origin-center`}>
